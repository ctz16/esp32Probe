...
As3933::As3933(SPIClass* spi, uint8_t ss): _spi(spi), _ss(ss)
{
...
bool As3933::setBitDuration(uint8_t rcRatio)
{
    if(--rcRatio>0x1F)
    {
        return false;
 ...
 */
// unsigned long As3933::antennaTuning(uint8_t antennaNr)
// {
//     uint8_t r16=read(16);
...
bool As3933::setNrOfActiveAntennas(uint8_t number)
{
    uint8_t r0=read(0);
    switch(number)
    {
...
bool As3933::setWakeUpPattern(uint8_t *pattern16)
{
...
void As3933::write(uint8_t reg, uint8_t data) {
...
uint8_t As3933::read(uint8_t reg) {
    uint8_t retVal;
    digitalWrite(_ss, HIGH);
    _spi->beginTransaction(_spiSettings);
